/**
 * チャンク分割結果に対する Step 3（品質チェック）+ Step 4（プログラム置換）
 *
 * 入力: 結果_chunked_gemini-2.5-flash_combined.txt
 * Step 3: Gemini 2.5 Flash で品質チェック（文脈分析なし、文字起こし結果のみで判定）
...
- 置換行のタイムスタンプは文字起こし結果の該当行と完全に一致させてください`;
    ...
    const qualityFile = path.join(__dirname, '..', 'results', '結果_chunked_gemini-2.5-flash_step3_quality.txt');
    fs.writeFileSync(qualityFile, qualityCheck, 'utf-8');
    console.log(`\n→ ${qualityFile} に保存`);

    return { qualityCheck, qualityFile };
}
...
async function main() {
    console.log('===== チャンク分割結果の品質チェック + 置換 =====\n');
    ...
    const outputFile = path.join(__dirname, '..', 'results', '結果_chunked_gemini-2.5-flash_step4_corrected.txt');
